...
export const fetchAvilableJobs = createAsyncThunk(
  ...
  async ({ page }, { rejectWithValue }) => {
    try {
      ...
      if (!response.ok) {
        return rejectWithValue({ message: message || "Failed to fetch jobs." });
      }
      return {
        data,
        counts,
...
export const fetchJobsBasedOnStatus = createAsyncThunk(
  ...
  async ({ status, page }, { rejectWithValue }) => {
    try {
      ...
      if (!response.ok) {
        return rejectWithValue({
          message: message || "Failed to fetch the data.",
        });
      }
...
export const deleteJob = createAsyncThunk(
  ...
  async ({ id }, { rejectWithValue }) => {
    try {
      ...
      if (!response.ok) {
        return rejectWithValue({ message: message || "Failed to delete job." });
      }
      return { jobId, status };
    } catch (error) {
      return rejectWithValue({
...
export const viewJob = createAsyncThunk(
  ...
  async ({ jobId }, { rejectWithValue }) => {
    try {
      ...
      if (!response.ok) {
        return rejectWithValue({
          message: message || "Failed to mark the view count.",
        });
      }
      return { data, message };
    } catch (error) {
...
export const createJob = createAsyncThunk(
  ...
  async ({ body }, { rejectWithValue }) => {
    try {
      ...
      if (!response.ok) {
        return rejectWithValue({
          message: message || "Failed to create new job.",
        });
      }
