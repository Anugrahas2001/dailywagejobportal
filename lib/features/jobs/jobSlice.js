  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchAvilableJobs.fulfilled, (state, action) => {
        ...
        const { data, totalCount, counts, message } = action.payload;
        console.log(counts, data, "==================");
        jobsAdapter.setAll(state, data);
        state.successMessage = message;
        state.totalCount = totalCount;
        state.statusCounts = counts;
        console.log(current(state), "111CHECK THE CURRENT STAGE");
      })
      ...
      .addCase(createJob.fulfilled, (state, action) => {
        ...
        console.log("STATUS COUNTS:", state.statusCounts);
...
export const { jobUpdated, jobAdded, jobRemoved, clearJobsError } =
  jobsSlice.actions;
